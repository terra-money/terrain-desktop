...
export const localTerraPathConfigured = createState<boolean>(
  !!window.store.getLocalTerraPath(),
);
export const blockState = createState<TerrariumBlocks>({
  blocks: [],
  latestHeight: 0,
});
...
export const StateListeners = () => {
  useEffect(() => {
    ...
    ipcRenderer.on(NEW_LOG, async (_: any, log: string) => {
      if (logsState.length >= MAX_LOG_LENGTH) { logsState.set((p) => p.slice(1).concat(log)); } else logsState.merge([log]);
    });

    ipcRenderer.on(
      LOCAL_TERRA_IS_RUNNING,
      (_: any, isLocalTerraRunning: boolean) => {
    ...
    ipcRenderer.on(
      LOCAL_TERRA_PATH_CONFIGURED,
      (_: any, isLocalTerraPathConfigured: boolean) => {
