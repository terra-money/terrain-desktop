...
function AccountView(props : {wallet : Wallet}) {
    ...
    useEffect(() => {
        getBalance(props.wallet.key.accAddress).then(c => {
            let luna = c.get("uluna")
            if(luna) {
    ...
    useEffect(() => {
        listenToAccountTx(props.wallet.key.accAddress, (data : any) => {
            let tx = data as TxInfo
            let nTx = [...txInfos, tx]
            setTxInfos(nTx as never[]);
        })
    }, [txInfos])
    return (
        <button className="flex text-left justify-between px-4 py-2 border-b border-blue-900">
        <div className="flex">
          <div>
            <p className="text-xs font-bold text-blue-600 uppercase">
              Address
            </p>
            <p className="text-2xl">{props.wallet.key.accAddress}</p>
          </div>
          <div className="ml-6">
            <p className="text-xs font-bold text-blue-600 uppercase">
              Luna
